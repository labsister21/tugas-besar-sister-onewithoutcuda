...
2.  The Leader creates a log entry for the "joint configuration" (`config:joint:Cold,Cnew|Cnew`) which includes both the old and new sets of peers. Decisions during this phase require majorities from _both_ old and new configurations.
...
Communication between Raft nodes (server-to-server) for consensus purposes (like `RequestVote` and `AppendEntries`) is handled using \*_JSON-RPC_.
...
4.  **Run the Client:**
    ...
    docker run -it --rm --name raft_client --network tugas-besar-sister-onewithoutcuda_raftnet --cap-add=NET_ADMIN raft-client node1:8081
    ```
